	...
	class BackupPerformer
	{
		...
		private void CopySingleFile(FolderFileItem itemBeingCopied, string source, string destination)
		{
			if (File.Exists(source))
			{
				...
				{
					shouldOverwrite = ShouldOverwrite(source, destination);
				}
				if (HasBeenCanceled)
				{
					return;
		...
		private void IterateOverFiles(List<FolderFileItem> paths, string backupDirectory)
		{
			try
			{
				...
				if (Directory.Exists(backupDirectory) || _isCalculatingFileSize)
				{
					...
					backupDirectory = Path.Combine(backupDirectory, "backup");
		...
		private bool ShouldOverwrite(string source, string destination)
		{
			return !LengthIsEqual(source, destination);
			//return !HashIsEqual(source, destination);
		}

		private bool LengthIsEqual(string source, string destination)
		{
		...
		private bool HashIsEqual(string source, string destination)
		{
			string sourceHash;
			...
			using (var md5 = MD5.Create())
			{
				...
				{
					destinationHash = BitConverter.ToString(md5.ComputeHash(stream)).Replace("-", "").ToLower();
				}
			}
